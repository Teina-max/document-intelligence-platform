...
1. **NEVER write to the protected production Supabase project**. The
   ref is listed in the local `.claude/rules/supabase-prod-protection.md`
   file (kept out of the public repo — if you cloned this from GitHub
   and don't have it, create it with your forbidden ref before doing
   any Supabase work). The rule covers SQL, CLI, REST, Edge Functions,
   keys, tokens. Even read-only checks are forbidden.
...
- Check the local `.claude/rules/supabase-prod-protection.md` (kept
  out of git) for the forbidden vs approved Supabase refs.
...
- UI/CLI output: French. Code, commits and identifiers: English.
...
| Forbidden Supabase ref | declared in the local `.claude/rules/supabase-prod-protection.md` (not committed) |
